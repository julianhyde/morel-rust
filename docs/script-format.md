...
Most of Morel's tests are scripts in `tests/script`.
...
the rules by which output is matched and generated. It is a contract
shared with the other implementations of Morel (morel-java,
morel-go), which run the same scripts.
...
`ScriptRunner`, run in idempotent mode, is the harness. It reads the
script, strips the output lines (remembering each statement's
expected output by position), runs the statements one by one, and
writes the regenerated script. For each statement it buffers the
output, then compares it with the expected output:
...
`ScriptTest` runs one script, writing the regenerated script under
`target/test-output` and failing with a diff if it differs from the
source. `tests/smile.rs` is the test that runs every `.smli` and
`.sml` file under `tests/script`, one test per script.
...
source, and prints a diff if they differ. Without `test`, `cargo run
-- tests/script/simple.smli` prints the regenerated script to
standard output. The suite is `cargo test`, and the script tests
alone are `cargo test --test smile`. To write a new test, put a
...
The Java and Go implementations keep copies of the scripts and run
them the same way; a script that they run should keep the same text
in every implementation.

[Morel Java](https://github.com/hydromatic/morel) keeps its scripts
in `src/test/resources/script`. To run one, writing the regenerated
script to `.out` next to the source and printing a diff if they
differ:
...
The suite is `ScriptTest`, a JUnit test.
...
`src/shell/output_matcher.rs` decides whether an actual output and an
expected output are equivalent. It is used only in lenient mode,
which is the default; setting `matchStrict` to `true` requires the two
to be identical, which is useful when a script is testing the printer
itself.
...
those tests are Rust unit tests, in `src/shell/output_matcher.rs`:
matching, and raw string generation. The script `idempotent.smli`
tests the parts of the format that can be tested from within a
script, such as which bindings are printed and the raw string literal
cases.
