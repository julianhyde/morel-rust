...
Three styles are recognised by the Datalog parser:
...
# This is a hash comment (Souffle-style)

.decl edge(x:int, y:int)  /* Inline comment */
```

(Note: Morel-style `(* ... *)` comments are not supported inside
Datalog programs — the embedded language uses C-style comment
syntax to match Souffle.)
