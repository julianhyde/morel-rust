...
impl Opt {
    ...
    pub(crate) fn compose(
        ...
        let g_result = g.eval_f1(r, frame, a)?;
        match g_result {
            Val::Some(v) => {
                let f_result = f.eval_f1(r, frame, &v)?;
    ...
    pub(crate) fn compose_partial(
        ...
        let g_result = g.eval_f1(r, frame, a)?;
        match g_result {
            Val::Some(v) => f.eval_f1(r, frame, &v),
            Val::Unit => Ok(Val::Unit),
            _ => panic!("Expected option from g, got {:?}", g_result),
        }
    }
