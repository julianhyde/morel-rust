...
impl TypeResolver {
    ...
    fn deduce_query_type(
        ...
        let env_rc = env.bind_all(&[]); // Create an Rc<dyn TypeEnv> from the borrowed env
    ...
    fn deduce_scan_step_type(
        ...
        span: &Span,
        _field_vars: &mut Vec<Var>,
        from_steps: &mut Vec<Step>,
    ) -> Result<Triple, Error> {
        ...
        // Build the new element type by combining previous fields with new
        // bindings.
        let mut label_terms = BTreeMap::new();

        // Get fields from the previous element type if it's a record.
        let prev_is_empty = if let Some(sequence) =
            self.variable_to_sequence(&triple.v)
            && let Some(field_list) = Self::field_list(&sequence)
        {
            for (i, field_name) in field_list.iter().enumerate() {
                let label = Label::from(field_name.clone());
                label_terms.insert(label, sequence.terms[i].clone());
            }
            field_list.is_empty()
        } else {
            true
        ...
        // Add new fields from the pattern.
        for (name, term) in &term_map {
            let label = Label::from(name.clone());
            label_terms.insert(label, term.clone());
        }

        // Create the new element type. If this is the first step (previous is
        // empty) and we only have one binding, use just that binding's type
        // instead of creating a record.
        let v = self.variable();

        if prev_is_empty && term_map.len() == 1 {
            // Just use the single bound variable's type
            self.equiv(&term_map[0].1, &v);
        } else {
        ...
        // (is_list_or_bag_matching_input, is_list_if_both_are_lists, may_be_bag_or_list)
        let c = self.variable();
        self.list_term(Term::Variable(v.clone()), &c);

        let step2 = Step {
            kind: StepKind::Scan(Box::new(pat2), Box::new(expr2), condition2),
            span: span.clone(),
        };
        from_steps.push(step2);
    ...
    fn deduce_yield_step_type(
        ...
        // Output is ordered iff input is ordered. Yield behaves like a 'map' function
