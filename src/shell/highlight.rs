...
mod tests {
    ...
    #[test]
    fn test_concise_classes() {
        // The classes are finer than the categories above: a name being
        // bound differs from one being used, punctuation from operators.
        assert_eq!(
            highlight_concise("val x = 1;"),
            "kr{val} nv{x} p{=} mi{1}p{;}"
        );
        assert_eq!(
            highlight_concise("fun f x = x + 1"),
            "kr{fun} nf{f} n{x} p{=} n{x} o{+} mi{1}"
        );
        assert_eq!(
            highlight_concise("from `from` in [1,2];"),
            "kr{from} nv{`from`} kr{in} p{[}mi{1}p{,}mi{2}p{];}"
        );
        assert_eq!(
            highlight_concise(r#""hi" (* c *) 'a List.map"#),
            r#"s2{"hi"} c{(*}cm{ c *)} nn{'a} nn{List}p{.}n{map}"#
        );
    }
